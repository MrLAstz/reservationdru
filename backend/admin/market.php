...
<script>
    var in_content = document.getElementById('in_content');
    in_content.addEventListener('load', function() {
       var url = this.contentWindow.location.toString(); 
       //alert(url);
        if (url.indexOf('index') >= 0) {
            in_content.src = "about:blank";
            jQuery('#modal_box').modal('hide');
            
            window.location.reload();
        } else {            
            //jQuery('#modal_box').modal('show');
            //in_content.style.display = 'block';
        }
    });
    ...
    document.addEventListener('DOMContentLoaded', function(ev) {    
        jQuery('[data-box]').click(function() {
            ...
            in_content.src = "in_renter.php?name="+box;
            jQuery('#modal_box').modal('show');
            in_content.style.display = 'block';            
        });
    });
